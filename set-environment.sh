...
WWWDOMAIN="www.gfd-test.local"
HTUSER=gfd
HTPASS=greym00n
APPNAME="gfd"
APPNAMELONG="GFD Test"
DATABASENAME="gfd"
DATABASEUSER="gfd"
...
writevalues() {
    ...
    echo "HTUSER=${HTUSER}" 
    echo "HTPASS=${HTPASS}" 
    echo "NGINXREDIRECTHTTP=${NGINXREDIRECTHTTP}" 
    echo "NGINXREDIRECTAPEX=${NGINXREDIRECTAPEX}" 
    echo "UPDATEPKGS=${UPDATEPKGS}" 
...
while [[ $# -gt 0 ]]; do
  ...
  -h | --help)
    ...
    echo "--updatepkgs"
    echo "     Update packages (apt upgrade) at build time (currently set to ${UPDATEPKGS})"
    echo "--clobber"
